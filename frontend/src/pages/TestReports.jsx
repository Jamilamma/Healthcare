...
export default function TestReports() {
  ...
  // FIX: was { type: "blood_test", ... } — key must be "reportType" to match schema & POST body
  ...
  const addReport = async () => {
    ...
    try {
      ...
      // FIX: reset uses correct key "reportType"
      setShowForm(false);
      setForm({ reportType: "blood_test", title: "", description: "" });
      const { data: res } = await api.get(`/patient/reports/${id}`);
      setData(res.data);
    } catch (e) { toast.error(e.response?.data?.message || "Failed"); }
  };
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div>
          <h1 className="font-bold text-gray-900 text-xl">Test & Reports</h1>
          <p className="text-xs text-gray-500">OTP protected · View only</p>
        </div>
      </div>
          ...
          {expiry && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2 text-sm text-green-700 flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                Session expires {expiry.toLocaleTimeString()}
              </span>
              {["admin", "doctor", "lab_technician"].includes(user?.role || "") && (
                ...
                >
                  <Upload className="w-3.5 h-3.5" />Add Report
                </button>
              )}
            </div>
          )}
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="inp"
                    placeholder="e.g. HbA1c Report Mar 2025"
                  />
                </div>
                <div>
                ...
                <div className="flex gap-2">
                  <button onClick={addReport} className="btn-blue">Save</button>
              ...
              <div key={r._id} className="border rounded-xl p-3 mb-2 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    By {r.uploadedBy} · {new Date(r.uploadedAt).toLocaleDateString()}
                  </p>
